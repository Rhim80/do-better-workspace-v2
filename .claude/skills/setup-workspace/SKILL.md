...
핵심 단계: **루트 확인 → 프로필 → Python 환경 → 선택 도구 → 저장 위치 확인 → 폰·수집 채널 확인 → 자리 선언표 → 첫 Daily Note**.
...
이 워크스페이스에는 앞으로 카톡·문자·메일 **원문이 그대로** 쌓인다. git 원격(GitHub 등)에 올리는 순간 그 내용이 내 컴퓨터 밖의 서버로 복사되고, 저장소가 **공개(public)** 상태면 인터넷의 누구나 볼 수 있다. 그래서 세팅 때 딱 한 번, 기계가 검사한다:
...
> 왜 자동 채널 1개가 최소 조건인지, 조합·제공자별 성립 근거는 `00-system/03-guides/수집기-약속.md`. 새 수집기를 붙일 땐 `/collector-check`로 검사표 7문을 통과시킨다.
...
- 새 수집기를 붙일 땐 `/collector-check`로 검사표 7문을 통과시킨다 (기준: `00-system/03-guides/수집기-약속.md`).
