...
# 자리 선언표·내 세계 사전 (reply-draft가 읽는 개인 층 — 회신 원칙·진행표 표면)
...
### 6. 자리 선언표·내 세계 사전 안내 (개인 층)

`reply-draft`(회신 초안)가 **자리 선언표**(`00-system/08-registry/자리-선언표.yaml`)의 진행표 표면과 **내 세계 사전**(`내-세계-사전.yaml`)의 `회신-원칙` 절(말투·금지 표현·첨부·서명)을 읽는다. 여기서는 안내만 한다:

- **[빈칸]은 지금 다 채우지 않는다.** 세팅은 질문지가 아니라 **실물에 잣대 대기** — 내 업무의 중심 표·회신 말투·금지 표현은 실물을 열어 놓고 하나씩 채운다. 쓰다가 "이건 항상 이렇게 해줘"라고 교정하는 순간이 오면 그때 사전에 한 줄씩 쌓인다 (그게 정상 속도).
- 비어 있어도 `reply-draft`는 기본 원칙만으로 동작한다 — 채우면 내 말투·금지 규칙이 자동 적용된다.
...
3. "새 프로젝트 시작" → do-better-drive로 목적→완료 7단계 진행
4. "카톡 읽어줘" / "카톡 체크" (Mac·윈도우) → 본인 카톡 요약·할일 추출
