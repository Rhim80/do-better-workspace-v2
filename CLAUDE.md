...
- **reply-draft** — 인바운드 회신 초안 전문 생성 + 내 원칙(사전 `내-세계-사전.yaml` `회신-원칙` 절: 말투·금지 표현·첨부·서명)으로 자동 검증. 발송은 사람이 "보내줘" 후에만. 진행표 표면이 선언돼 있으면 발송 후 행 갱신을 제안한다.
...
**관심사**:
**이 워크스페이스 용도**:
**판단 성향**: <!-- 예: 창작 프로젝트를 고를 때 "나답나"를 먼저 보나 "돈이 되나"를 먼저 보나. Do Better Drive(do-better-drive)가 이 줄을 읽어 기본값 대신 따른다. 비워두면 기본값 사용. -->
