...
프로젝트를 "목적 → 완료"까지 단계로 몰아가는 방법. 각 단계는 `progress.md`의 자기 칸을 채우고 다음으로 넘긴다. 개발자용 gstack의 비개발 버전. **한 스킬 `do-better-drive`가 7단계를 progress.md `status`로 라우팅한다** (단계 상세는 스킬 내부 `references/1~7`). (형식은 `10-projects/README.md`의 "Do Better Drive 진행" 참조.)
...
| 스킬 | 용도 | 트리거 예시 |
|------|------|-------------|
| `do-better-drive` | 프로젝트를 목적→완료 7단계로 구동. progress.md `status`로 단계 라우팅, 매 단계 게이트에서 멈춤. | "새 프로젝트 시작", "검토", "회고", "내보내기" |
...
| ④ 생산 | 변형→비교→확정→풀생산 | "생산 시작", "변형 뽑아 비교" |
...
- `do-better-drive` ④생산 → 확정된 방향을 기존 콘텐츠·산출 스킬로 풀생산 (재구현하지 않고 호출)
- `do-better-drive` ⑥내보내기 → 발행·발송을 기존 스킬(`ghost-publish`·`gmail`·`slack`·`naver-commerce`·`note-publish` 등)에 위임 (재구현하지 않고 호출)
