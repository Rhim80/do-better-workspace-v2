...
| 스킬 | 용도 | 트리거 예시 |
|------|------|-------------|
| `collect` | 선언표의 수집기 중 실행주기 된 것을 돌려 raw/에 쌓기 | "수집 돌려", "collect" |
...
| `morning` | 아침 상태판 — 수집기 신선도 + 표면 점검 + 이어가기 (선언표를 읽고 돈다) | "morning", "좋은 아침" |
| `collector-check` | 새 채널을 붙이기 전 수집기 약속 6문 검사 | "수집기 검사", "collector-check" |
...
| `ledger-reconcile` | 정형 입구(거래내역·export) 발행↔입금 매칭 → 장부 갱신 — 패턴·임계값은 사전 `장부` | "장부 맞추기", "입금 매칭" |

### Do Better Drive (프로젝트 구동 — 목적→완료 7단계)

프로젝트를 "목적 → 완료"까지 단계로 몰아가는 방법. 각 기어는 `progress.md`의 자기 칸을 채우고 다음 단계로 넘긴다. 개발자용 gstack의 비개발 버전. **1~7단계 전부 구현.** (형식은 `10-projects/README.md`의 "Do Better Drive 진행" 참조.)

| 스킬 | 단계 | 트리거 예시 |
|------|------|-------------|
| `drive-frame` | 1 · 프레임 (목적·대상 확정) | "새 프로젝트 시작", "이거 할까 말까" |
| `drive-scope` | 2 · 될 일인가 (효과 실현성) | "이거 될 일인가", "실현성 점검" |
| `drive-plan` | 3 · 계획 (구조·합격 기준) | "계획 짜자", "구조 잡자" |
| `drive-make` | 4 · 생산 (변형→비교→확정→풀생산) | "생산 시작", "변형 뽑아 비교" |
| `drive-review` | 5 · 냉정한 검토 (합격 기준 대조 + 계획 이행 감사) | "검토", "이거 됐나 봐줘", "합격인가" |
| `drive-ship` | 6 · 내보내기 (유통 경로로 발행·발송) | "내보내기", "발행하자", "모객 시작" |
| `drive-retro` | 7 · 돌아보기·배우기 (교훈 포착→복리) | "돌아보기", "회고", "배운 것 뽑아줘" |
...
- `drive-frame` → `drive-scope` → `drive-plan` → `drive-make` → `drive-review` → `drive-ship` → `drive-retro` (프로젝트 `progress.md`를 통해 단계별로 이어짐. 검토에서 막히면 앞 단계로 되돌림)
- `drive-make`(4단계) → 확정된 방향을 기존 콘텐츠·산출 스킬로 풀생산 (재구현하지 않고 호출)
- `drive-ship`(6단계) → 발행·발송을 기존 스킬(`ghost-publish`·`gmail`·`slack`·`naver-commerce`·`note-publish` 등)에 위임 (재구현하지 않고 호출)
- `drive-retro`(7단계) → 재사용할 지식을 `wiki-ingest`로 `00-wiki`에 축적 (지식 복리)
